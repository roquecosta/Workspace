---
name: project-pd-brcr-billing-rate-card-report
description: Projeto pd_brcr (StoccheForbes) — Relatório de Cartões de Taxa de Cobrança; caminho do MANIFEST, seção nova para lib compartilhada externa e padrão para record nativo referenciado com field IDs pendentes
metadata:
  type: project
---

Projeto **pd_brcr — Relatório de Cartões de Taxa de Cobrança**, cliente **StoccheForbes**.

MANIFEST em:
`C:\Users\Roque\Documents\ProjectDome\StoccheForbes\src\FileCabinet\SuiteScripts\project_dome\pd_brcr_billing_rate_card_report\Docs\MANIFEST.md`
...
Os dois scripts (`customscript_pd_brcr_report_sl`, `customscript_pd_brcr_report_rl`) têm Script ID
